...
def test_410_gone_disables_endpoint_without_recording_success():
    manager = WebhookManager()
    endpoint = manager.register_endpoint(
        workspace_id="workspace-a",
        url="https://hooks.example.com/agent-events",
    ...
    record = manager.deliver(
        ...
        event_type="run.completed",
        event_id="evt-gone",
        payload={"run_id": "run-public-1", "internal_run_id": "secret"},
        sender=lambda url, payload: callbacks.append(payload) or 410,
    )

    disabled = manager.get_endpoint("workspace-a", endpoint.id)
    assert record.status == "gone"
    assert record.response_status == 410
    assert record.error == "endpoint returned 410 Gone"
    assert record.payload == {"run_id": "run-public-1"}
    assert disabled is not None
    assert disabled.enabled is False
    assert disabled.disabled_reason == "http_410_gone"
    assert callbacks == [{"run_id": "run-public-1"}]
...
def test_410_retry_is_idempotent_and_does_not_callback_again():
    manager = WebhookManager()
    endpoint = manager.register_endpoint(
        workspace_id="workspace-a",
        url="https://hooks.example.com/agent-events",
    ...
    first = manager.deliver(
        ...
        event_type="run.completed",
        event_id="evt-gone",
        payload={"run_id": "run-public-1", "trace_id": "secret"},
        sender=lambda url, payload: callbacks.append(payload) or 410,
    )
    retry = manager.deliver(
        workspace_id="workspace-a",
        ...
        event_type="run.completed",
        event_id="evt-gone",
        payload={"run_id": "run-public-1", "trace_id": "changed"},
        sender=lambda url, payload: callbacks.append(payload),
    )

    assert retry.delivery_id == first.delivery_id
    assert retry.status == "gone"
    assert callbacks == [{"run_id": "run-public-1"}]


def test_sender_response_objects_are_supported():
    manager = WebhookManager()
    endpoint = manager.register_endpoint(
        workspace_id="workspace-a",
        url="https://hooks.example.com/agent-events",
    ...
    record = manager.deliver(
        ...
        event_id="evt-1",
        payload={"run_id": "run-public-1"},
        sender=lambda url, payload: WebhookDeliveryResponse(status_code=202),
    )
