...
from .auth import AuthError, permissions
...
class AuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(
        ...
        if request.url.path.startswith("/api/v2") and path not in public_paths:
            try:
                request.state.principal = permissions.authorize_request(
